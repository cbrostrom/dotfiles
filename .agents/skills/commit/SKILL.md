...
- **No attribution**: Commit message is subject + optional body only. Never append `Made with Cursor`, `Made-with:`, `Co-authored-by:`, `Co-Authored-By:`, or any AI/tool attribution line. Cursor may inject these anyway; the global git hook strips them, but do not write them yourself.
...
Run `git status` after the commit to verify success. Report the commit hash(es) and subject line(s). If the commit message still contains attribution after the hook runs, amend with a cleaned message (only when safe per git policy).
