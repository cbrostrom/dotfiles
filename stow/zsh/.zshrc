...
# Resolve the repository from the Stow package source.
ZSHRC_PATH="${(%):-%N}"
[[ -L "$HOME/.zshrc" ]] && ZSHRC_PATH="$(readlink -f "$HOME/.zshrc")"
DOTFILES_DIR="$(cd "$(dirname "$ZSHRC_PATH")/../.." && pwd)"
...
# OBSIDIAN_API_KEY and other rbw secrets are loaded by .zshenv.
