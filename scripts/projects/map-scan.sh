...
_run_inventory() {
    ...
    "$BASH" "$SCRIPT_DIR/inventory.sh" --no-clean-size || {
        [[ -f "$OUT_JSON" ]] || {
            err "inventory.sh failed and $OUT_JSON was not written"
            exit 1
        }
...
while IFS=$'\t' read -r rel name stack flags; do
    ...
    has_codebase "$abs" && {
        codebase_yn="yes"
        with_codebase=$((with_codebase + 1))
    }
    codebase_local_yn="no"
    has_codebase_local "$abs" && {
