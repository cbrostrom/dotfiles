#!/usr/bin/env bash
# Shared RTK command rewrite policy.
# Reads a shell command from argv or stdin and prints the RTK-prefixed command
# when a supported command family matches. Prints nothing when no rewrite applies.
set -euo pipefail
...
# Heredocs are too easy to corrupt with simple shell rewriting.
case "$FIRST_CMD" in
  *'<<'*) exit 0 ;;
esac

# Strip leading env var assignments for matching, but preserve them in output.
