...
# Device snapshot (.claude/devices/<host>.json) is regenerated by the
# pre-commit hook when committing .claude/ changes — not here. Running on
# every install left a permanently dirty worktree because the file's
# last_synced timestamp updated each run.
