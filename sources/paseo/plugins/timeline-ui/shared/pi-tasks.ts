...
/**
 * Structural equality of two task lists, used by the timeline renderer to
 * collapse superseded task-list cards (older `todo` updates) into nothing so
 * only the latest card stays visible.
 */
export function sameTaskList(
  ...
  return a.every((task, index) => {
    const other = b[index];
    if (task.id !== other.id) return false;
    if (task.text !== other.text) return false;
    if (task.status !== other.status) return false;
    if (task.activeForm !== other.activeForm) return false;
    return true;
  });
}
