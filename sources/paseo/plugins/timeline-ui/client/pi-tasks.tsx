...
import { activeTasks, usePiTaskSnapshot } from "./pi-tasks-state.js";
import { piTaskListSchema, sameTaskList } from "../shared/pi-tasks.js";
...
/**
 * Timeline card for a `todo` tool-call. Every task update produces its own
 * card, so only the card matching the agent's latest task snapshot renders in
 * full; superseded (older) cards render nothing. When the snapshot is missing
 * or still loading, the card renders as a safe fallback.
 */
export function PiTaskList({
  ...
  const superseded =
    snapshot.tasks !== null && !sameTaskList(item.data.tasks, snapshot.tasks);
