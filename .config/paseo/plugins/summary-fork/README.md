...
Paseo plugin that forks a chat into a fresh conversation in the same workspace using a model-generated, editable summary — no Paseo patching, no temporary files.

## Flow

1. **Fork** pill (or ⌘K "Fork with summary") opens the panel.
2. Pick scope: since last compaction (default) / full / last 10-25-50 turns.
...
7. **Create fork** — new agent, same provider/model/mode/thinking, receiving only the summary as a persisted virtual text attachment (`contextKind: "fork-summary"`). The panel opens the new chat.
