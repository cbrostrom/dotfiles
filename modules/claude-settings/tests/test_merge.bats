...
setup() {
    ...
    export MERGE_SKIP_ENVSUBST=1
}

teardown() { rm -rf "$TMPDIR"; }

@test "merge: darwin platform produces expected merged output" {
    MERGE_SKIP_ENVSUBST=1 CLAUDE_DIR="$TMPDIR" PLATFORM=darwin "$MOD_DIR/merge.sh"
...
@test "merge: linux platform produces expected merged output" {
    MERGE_SKIP_ENVSUBST=1 CLAUDE_DIR="$TMPDIR" PLATFORM=linux "$MOD_DIR/merge.sh"
    diff <(jq -S . "$TMPDIR/settings.local.json") \
...
@test "merge: invalid JSON in base aborts without touching settings.local.json" {
    ...
    run env MERGE_SKIP_ENVSUBST=1 CLAUDE_DIR="$TMPDIR" PLATFORM=darwin "$MOD_DIR/merge.sh"
    [ "$status" -ne 0 ]
